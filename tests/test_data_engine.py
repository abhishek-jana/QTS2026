...
def test_pit_isolation():
    ...
    record_2 = {
        ...
        'open': 100.0, 'high': 100.0, 'low': 100.0, 'close': 105.0, 'volume': 1000,
        'is_correction': True
    }
    
    # Ingest data
    engine.conn.execute("DELETE FROM market_data")
    df = pd.DataFrame([record_1, record_2])
    engine.insert_dataframe(df)
