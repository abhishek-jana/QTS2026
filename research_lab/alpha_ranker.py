...
class LightweightViT(nn.Module):
    def __init__(self, img_size=(32, 63), patch_size=(32, 9), in_chans=1, embed_dim=64, depth=2, num_heads=4, dropout=0.1):
        super().__init__()
            ...
            for epoch in range(epochs):
                ...
                for b in range(nb):
                    ...
                    with torch.amp.autocast('cuda', enabled=(device.type == 'cuda')):
                        ini = {k: v[idx][ii].detach() for k, v in dataset.data.items()}; inj = {k: v[idx][jj].detach() for k, v in dataset.data.items()}
                ...
                if val_dataset:
                    ...
                    with torch.no_grad(), torch.amp.autocast('cuda', enabled=(device.type == 'cuda')):
                        ...
                        # Use a small random sample from validation set for speed
                        ...
                        if len(v_idx) >= 2:
                            ...
                            v_si, v_sj = self.forward(v_ini), self.forward(v_inj)
                            v_loss = criterion(v_si, v_sj, torch.sign(val_dataset.labels[v_idx][vii] - val_dataset.labels[v_idx][vjj]).unsqueeze(1))
