  ...
  return (
    <div className="flex-1 overflow-y-auto no-scrollbar pr-2 h-[450px]">
      <div className="flex justify-between items-center mb-2 border-b border-emerald-900/30 pb-1">
        <div className="text-lg font-bold text-emerald-500/80 uppercase tracking-tighter italic">Decile Ladder (House View)</div>
        {filterSector && (
          <button onClick={onClearFilter} className="flex items-center gap-1 bg-emerald-500/10 border border-emerald-500/30 px-2 py-0.5 rounded-sm hover:bg-emerald-500/20 transition-all">
            <span className="text-lg font-bold text-emerald-400 uppercase tracking-widest">{filterSector}</span>
            <X className="w-2 h-2 text-emerald-400" />
          </button>
        )}
      </div>
      <table className="w-full text-lg border-collapse font-mono">
        ...
        <tbody>
          {filtered.map((row) => (
            <tr key={row.ticker} onClick={() => onSelectTicker(row.ticker)} className="border-b border-slate-800/40 hover:bg-emerald-500/10 cursor-pointer group transition-all">
              ...
              <td className="text-right py-2"><span className={"text-lg px-1 border " + (row.score > 0 ? "border-emerald-800 text-emerald-600" : "border-red-900 text-red-700") + " uppercase font-black"}>{row.score > 0 ? "Long" : "Short"}</span></td>
            </tr>
          ))}
        </tbody>
      </table>
...
const Panel = ({ title, icon: Icon, children, className = "" }) => (
  <div className={"bg-slate-900 border border-slate-700 p-4 flex flex-col h-fit shadow-lg " + className}>
    <div className="flex-none flex items-center gap-2 mb-4 border-b border-slate-800 pb-2">
      ...
      <h2 className="text-lg font-mono uppercase tracking-[0.2em] text-slate-300 font-bold">{title}</h2>
    </div>
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-md p-4 font-mono">
      <div className="bg-slate-900 border border-emerald-500/50 w-full max-w-6xl max-h-[95vh] overflow-hidden flex flex-col shadow-2xl">
        <div className="p-5 border-b border-slate-800 flex justify-between items-center bg-emerald-500/10">
          <div className="flex items-center gap-3 text-emerald-400 font-bold uppercase tracking-[0.3em] text-lg">
        ...
        <div className="p-10 space-y-12 overflow-y-auto no-scrollbar bg-gradient-to-b from-slate-900 to-black">
          {sections.map((s, i) => (
            <div key={i} className="space-y-6">
              <h3 className="text-emerald-500 font-black uppercase text-lg tracking-[0.4em] border-b border-emerald-900/40 pb-3 flex items-center gap-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {s.items.map((it, j) => (
                  <div key={j} className="bg-slate-800/10 p-5 border border-slate-700/30 group hover:border-emerald-500/50 transition-all hover:bg-slate-800/30 shadow-2xl">
                    <h4 className="text-slate-100 font-bold text-lg uppercase mb-3 flex items-center gap-2">
                      <div className="w-2 h-2 bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.8)]" />
                      {it.label}
                    </h4>
                    <p className="text-slate-400 text-lg leading-relaxed font-medium group-hover:text-slate-200">{it.detail}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="p-8 bg-slate-900 border-t border-slate-800 flex justify-between items-center">
          <div className="space-y-1">
            <div className="text-lg text-emerald-500 font-black uppercase tracking-widest flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-emerald-500 animate-ping" />
              SYSTEM_HEARTBEAT: NOMINAL
            </div>
            <div className="text-lg text-slate-600 uppercase font-bold tracking-widest italic">C++26 OSQP Kernel // SIP_STREAM V2.5</div>
          </div>
          <button onClick={onClose} className="bg-emerald-600 text-black px-20 py-4 font-black uppercase text-lg hover:bg-emerald-400 shadow-[0_0_30px_rgba(16,185,129,0.4)] transition-all active:scale-95 border-none">
...
export default function MissionControl() {
  ...
  return (
    <ErrorBoundary>
      ...
      <div className="min-h-screen w-full bg-black text-slate-300 font-mono text-lg flex flex-col p-2 gap-2 overflow-x-hidden selection:bg-emerald-500/30">
        ...
        <div className="flex-none h-16 flex justify-between items-center border-b border-slate-800 pb-2 px-2">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 border-l border-slate-800 pl-4"><div className={"w-2 h-2 rounded-full " + (status === "active" ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.8)]" : "bg-red-500")} /><span className="uppercase text-lg font-bold text-slate-500">{status}</span></div>
            <div className="text-lg text-slate-600 border border-slate-800 px-2 py-0.5 rounded-sm uppercase tracking-tighter">SIM_TIME: {globalData.timestamp}</div>
            <div className="flex gap-6 px-6 border-l border-slate-800">
               <div className="flex flex-col"><span className="text-lg text-slate-600 uppercase font-black tracking-widest">Gross Exp</span><span className="text-lg font-bold text-slate-100 italic">{(inst.gross_exposure || 0).toFixed(1)}%</span></div>
               <div className="flex flex-col"><span className="text-lg text-slate-600 uppercase font-black tracking-widest">Net Exp</span><span className="text-lg font-bold text-slate-100">{(inst.net_exposure || 0).toFixed(1)}%</span></div>
               <div className="flex flex-col border-l border-slate-800 pl-6"><span className="text-lg text-slate-600 uppercase font-black tracking-widest">Latency</span><span className="text-lg font-bold text-emerald-500 italic">{(inst.data_latency_ms || 0).toFixed(0)}ms</span></div>
               <div className="flex flex-col border-l border-slate-800 pl-6"><span className="text-lg text-slate-600 uppercase font-black tracking-widest">Freshness</span><span className="text-lg font-bold text-slate-400">{(inst.data_freshness_s || 0).toFixed(0)}s</span></div>
            </div>
            <button onClick={() => setShowManual(true)} className="flex items-center gap-1.5 px-3 py-1 border border-emerald-900/30 hover:bg-emerald-500/10 transition-all text-slate-500 hover:text-emerald-400 group ml-4 outline-none"><HelpCircle className="w-3 h-3" /><span className="text-lg uppercase font-bold tracking-[0.2em]">Manual</span></button>
          </div>
          <button onClick={() => ws.current.send('KILL_SWITCH')} className="px-6 py-2 transition-all font-black uppercase text-lg border border-red-500/50 text-red-500 hover:bg-red-500 hover:text-white shadow-[0_0_15px_rgba(220,38,38,0.2)]">Kill System</button>
        </div>
        ...
        <div className="flex-1 grid grid-cols-12 gap-2 pb-4 overflow-hidden">
          ...
          <Panel title="Metacognition" icon={ShieldAlert} className="col-span-4 h-full overflow-y-auto no-scrollbar">
            <div className="flex flex-col gap-8">
               <div className="flex justify-between items-center bg-slate-800/30 p-8 border border-slate-700 shadow-2xl"><div className="flex flex-col"><span className="text-lg text-slate-600 uppercase font-black tracking-widest">Bayesian Belief</span><span className="text-6xl font-black text-emerald-400 drop-shadow-[0_0_15px_rgba(16,185,129,0.3)]">{((meta.belief_score || 0) * 100).toFixed(2) + "%"}</span></div><Gauge className="w-16 h-16 text-emerald-500 opacity-20" /></div>
               <div className="h-72 border border-slate-800 bg-black/40 p-4">
                 <div className="text-lg text-slate-600 mb-4 uppercase font-black text-center tracking-[0.4em]">Manifold Drift (t-SNE Latent)</div>
          ...
          <Panel title="Ranking Ladder" icon={TrendingUp} className="col-span-4 h-full overflow-hidden flex flex-col">
             ...
             <div className="flex-1 border-t border-slate-800 mt-4 pt-4 overflow-y-auto no-scrollbar">
                <div className="text-lg text-slate-500 mb-2 uppercase font-black tracking-widest flex justify-between items-center"><span>Sector Matrix (Interactive)</span><Filter className="w-3 h-3 text-slate-700" /></div>
                <div className="grid grid-cols-3 gap-1.5 pb-4">
                  {Object.entries(inst.sector_exposure || {}).map(([sector, stats]) => (
                     <button key={sector} onClick={() => setSelectedSector(selectedSector === sector ? null : sector)} className={"border p-2 flex flex-col justify-center transition-all text-left " + (selectedSector === sector ? "bg-emerald-500/20 border-emerald-500" : "bg-slate-800/30 border-slate-800 hover:bg-slate-800/50 hover:border-slate-600")}>
                        <div className="text-lg text-slate-500 uppercase truncate font-bold">{sector}</div>
                        <div className={"text-lg font-black " + (stats.exposure > 0 ? "text-emerald-500" : "text-rose-500")}>{(stats.exposure > 0 ? "+" : "") + stats.exposure.toFixed(1)}%</div>
                        <div className="flex justify-between items-center mt-1 border-t border-slate-700/50 pt-1">
                            <span className="text-base text-slate-600 uppercase font-black">{stats.count} Tickers</span>
                            <span className="text-base text-emerald-600/60 italic font-black">α: {(stats.avg_score || 0).toFixed(2)}</span>
                        </div>
                     </button>
                  ))}
                </div>
                ...
                <div className="mt-4 pt-4 border-t border-slate-800 h-40">
                    <div className="text-lg text-slate-600 uppercase font-black mb-2 tracking-widest">L/S Equity Spread (Cumulative)</div>
                    <ResponsiveContainer width="100%" height="100%">
          ...
          <Panel title="Execution & Reality Check" icon={Cpu} className="col-span-12 mt-2 h-fit">
            <div className="grid grid-cols-12 gap-8 p-2">
              <div className="col-span-2 flex flex-col justify-center gap-2 border-r border-slate-800 pr-8">
                <div className="text-lg text-slate-500 uppercase font-black tracking-tighter leading-none mb-1">Implementation Shortfall</div>
                <div className="text-3xl font-black text-slate-100 italic">{(exec.implementation_shortfall || 0).toFixed(2)} <span className="text-lg text-slate-700 not-italic uppercase">BPS</span></div>
                <div className="text-lg text-slate-500 font-mono italic">{"IS_VAR_95: " + (exec.is_var || 0).toFixed(6)}</div>
              </div>

              {/* OMS Queue - Functional with SOLD info */}
              <div className="col-span-5 border-r border-slate-800 pr-8 flex flex-col gap-2">
                <div className="text-lg text-slate-500 uppercase font-black flex justify-between items-center tracking-widest border-b border-slate-800/50 pb-1">
                ...
                <div className="bg-black/50 p-2 h-64 overflow-y-auto no-scrollbar font-mono text-lg border border-slate-800 shadow-inner">
                    ...
                    {(!inst.order_log || inst.order_log.length === 0) && <div className="h-full flex items-center justify-center text-slate-800 italic uppercase tracking-[0.3em] animate-pulse">Awaiting execution commands...</div>}
                </div>
              </div>
              
              <div className="col-span-2 border-r border-slate-800 pr-8">
                 <div className="flex flex-col gap-2 w-full px-2">
                    <div className="text-lg text-slate-600 uppercase font-black tracking-widest text-center">Slippage</div>
              ...
              <div className="col-span-3 flex flex-col justify-center items-end text-right gap-4">
                 <div className="w-full space-y-1.5">
                    <div className="grid grid-cols-2 gap-2">
                        <div className="bg-slate-800/20 p-2 border border-slate-800 flex flex-col items-center justify-center"><span className="text-base text-slate-500 uppercase font-black">Champ</span><span className="text-lg font-black text-cyan-400">{(pipeline.champion_sharpe || 0).toFixed(2)}</span></div>
                        <div className="bg-slate-800/20 p-2 border border-slate-800 flex flex-col items-center justify-center"><span className="text-base text-slate-500 uppercase font-black">Chal</span><span className="text-lg font-black text-emerald-400">{(pipeline.challenger_sharpe || 0).toFixed(2)}</span></div>
                    </div>
                    <div className="bg-black/40 p-1 border border-slate-800 text-lg text-emerald-500/60 uppercase font-black truncate animate-pulse text-center tracking-widest">SYSTEM_PIPELINE: {pipeline.training_progress || 'V1_ACTIVE'}</div>
                 </div>

                 <div className="flex flex-col items-end gap-1">
                    <div className="text-lg text-slate-600 italic uppercase font-black tracking-tighter italic">Muscle: C++26</div>
                    <div className="text-emerald-500 font-black text-lg shadow-[0_0_15px_rgba(16,185,129,0.2)] border border-emerald-900/50 px-3 py-1 uppercase tracking-widest bg-emerald-950/20">Linked_Ready</div>
                    <div className="text-lg text-slate-400 font-mono mt-1 uppercase font-bold tracking-widest">Latency: 84.2μs</div>
                    <div className="text-lg text-slate-800 font-bold uppercase tracking-widest">Freshness: {(inst.data_freshness_s || 0).toFixed(0)}s</div>
                    <div className="text-lg text-slate-800 font-bold uppercase tracking-widest">Direct OSQP Context</div>
                 </div>
              </div>
            </div>
          </Panel>
