    ...
    if (this.state.hasError) {
      return (
        <div className="h-screen w-full bg-slate-900 text-red-500 p-10 font-mono flex flex-col items-center justify-center border-[20px] border-red-900/50">
          ...
          <button onClick={() => window.location.reload()} className="mt-8 bg-red-600 text-white px-12 py-3 font-black uppercase text-xs hover:bg-red-500 shadow-2xl transition-all">Reboot Cockpit</button>
        </div>
      );
    }
...
const Heatmap = ({ data, title }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col min-h-0">
      <div className="text-[10px] font-mono text-slate-500 mb-1 tracking-tighter truncate font-bold uppercase">{title}</div>
  ...
  return (
    <div className="flex-1 overflow-y-auto no-scrollbar pr-2 h-[450px]">
      <div className="flex justify-between items-center mb-2 border-b border-emerald-900/30 pb-1">
        <div className="text-[10px] font-bold text-emerald-500/80 uppercase tracking-tighter italic">Decile Ladder (House View)</div>
        {filterSector && (
        ...
        <div className="flex-none h-16 flex justify-between items-center border-b border-slate-800 pb-2 px-2">
          <div className="flex items-center gap-4">
            ...
            <div className="flex gap-6 px-6 border-l border-slate-800">
               ...
               <div className="flex flex-col border-l border-slate-800 pl-6"><span className="text-[8px] text-slate-600 uppercase font-black tracking-widest">Latency</span><span className="text-xs font-bold text-emerald-500 italic">{(inst.data_latency_ms || 0).toFixed(0)}ms</span></div>
               <div className="flex flex-col border-l border-slate-800 pl-6"><span className="text-[8px] text-slate-600 uppercase font-black tracking-widest">Freshness</span><span className="text-xs font-bold text-slate-400">{(inst.data_freshness_s || 0).toFixed(0)}s</span></div>
            </div>
        ...
        <div className="flex-1 grid grid-cols-12 gap-2 pb-4 overflow-hidden">
          ...
          <Panel title="Spectral Alpha" icon={Activity} className="col-span-4 h-full overflow-y-auto no-scrollbar">
            <div className="flex flex-col gap-6">
              ...
              L362:               <div className="h-48 border-t border-slate-800 pt-4"><div className="text-[8px] text-slate-600 mb-2 uppercase font-black tracking-widest text-right">Idiosyncratic SHAP Fusion</div>{spectralData?.shap_values ? (<ResponsiveContainer width="100%" height="100%"><BarChart data={Object.entries(spectralData.shap_values).map(([name, val]) => ({ name, val }))} layout="vertical" onClick={(data) => { if(data) alert(`MODALITY_OVERRIDE: Focus on ${data.activeLabel}`); }}><XAxis type="number" hide domain={[0, 1]} /><YAxis dataKey="name" type="category" width={110} tick={{ fontSize: 8, fill: '#475569' }} /><Tooltip contentStyle={{backgroundColor: '#0f172a', border: '1px solid #1e293b', fontSize: '9px'}} cursor={{fill: 'rgba(16,185,129,0.05)'}} /><Bar dataKey="val" fill="#10b981" radius={[0, 2, 2, 0]} className="cursor-pointer hover:fill-emerald-400 transition-colors" /></BarChart></ResponsiveContainer>) : <div className="h-full w-full flex items-center justify-center text-slate-800 italic uppercase text-[8px] tracking-widest">Awaiting MODALITY_GATING...</div>}</div>
            </div>
          </Panel>
          ...
          <Panel title="Metacognition" icon={ShieldAlert} className="col-span-4 h-full overflow-y-auto no-scrollbar">
            <div className="flex flex-col gap-8">
               ...
               <div className="h-72 border border-slate-800 bg-black/40 p-4">
                 <div className="text-[8px] text-slate-600 mb-4 uppercase font-black text-center tracking-[0.4em]">Manifold Drift (t-SNE Latent)</div>
          ...
          <Panel title="Ranking Ladder" icon={TrendingUp} className="col-span-4 h-full overflow-hidden flex flex-col">
             ...
             <div className="flex-1 border-t border-slate-800 mt-4 pt-4 overflow-y-auto no-scrollbar">
                ...
                <div className="grid grid-cols-3 gap-1.5 pb-4">
                  {Object.entries(inst.sector_exposure || {}).map(([sector, stats]) => (
                     <button key={sector} onClick={() => setSelectedSector(selectedSector === sector ? null : sector)} className={"border p-2 flex flex-col justify-center transition-all text-left " + (selectedSector === sector ? "bg-emerald-500/20 border-emerald-500" : "bg-slate-800/30 border-slate-800 hover:bg-slate-800/50 hover:border-slate-600")}>
                        <div className="text-[7px] text-slate-500 uppercase truncate font-bold">{sector}</div>
                ...
                <div className="mt-4 pt-4 border-t border-slate-800 h-40">
                    <div className="text-[8px] text-slate-600 uppercase font-black mb-2 tracking-widest">L/S Equity Spread (Cumulative)</div>
                    <ResponsiveContainer width="100%" height="100%">
          ...
          <Panel title="Execution & Reality Check" icon={Cpu} className="col-span-12 mt-2 h-fit">
            <div className="grid grid-cols-12 gap-8 p-2">
              <div className="col-span-2 flex flex-col justify-center gap-2 border-r border-slate-800 pr-8">
                ...
                <div className="text-3xl font-black text-slate-100 italic">{(exec.implementation_shortfall || 0).toFixed(2)} <span className="text-xs text-slate-700 not-italic uppercase">BPS</span></div>
              ...
              <div className="col-span-5 border-r border-slate-800 pr-8 flex flex-col gap-2">
                <div className="text-[10px] text-slate-500 uppercase font-black flex justify-between items-center tracking-widest border-b border-slate-800/50 pb-1">
                ...
                <div className="bg-black/50 p-2 h-32 overflow-y-auto no-scrollbar font-mono text-[9px] border border-slate-800 shadow-inner">
              ...
              <div className="col-span-2 border-r border-slate-800 pr-8">
                 <div className="flex flex-col gap-2 w-full px-2">
                    <div className="text-[9px] text-slate-600 uppercase font-black tracking-widest text-center">Slippage</div>
              ...
              <div className="col-span-3 flex flex-col justify-center items-end text-right gap-4">
                 <div className="w-full space-y-1.5">
                    <div className="grid grid-cols-2 gap-2">
                        <div className="bg-slate-800/20 p-2 border border-slate-800 flex flex-col items-center justify-center"><span className="text-[6px] text-slate-500 uppercase font-black">Champ</span><span className="text-[10px] font-black text-cyan-400">{(pipeline.champion_sharpe || 0).toFixed(2)}</span></div>
                        <div className="bg-slate-800/20 p-2 border border-slate-800 flex flex-col items-center justify-center"><span className="text-[6px] text-slate-500 uppercase font-black">Chal</span><span className="text-[10px] font-black text-emerald-400">{(pipeline.challenger_sharpe || 0).toFixed(2)}</span></div>
                    </div>
                    <div className="bg-black/40 p-1 border border-slate-800 text-[7px] text-emerald-500/60 uppercase font-black truncate animate-pulse text-center tracking-widest">SYSTEM_PIPELINE: {pipeline.training_progress || 'V1_ACTIVE'}</div>
                 </div>
                 ...
                 <div className="flex flex-col items-end gap-1">
                    ...
                    <div className="text-[8px] text-slate-800 font-bold uppercase tracking-widest">Freshness: {(inst.data_freshness_s || 0).toFixed(0)}s</div>
                    <div className="text-[8px] text-slate-800 font-bold uppercase tracking-widest">Direct OSQP Context</div>
                 </div>
              </div>
            </div>
          </Panel>
